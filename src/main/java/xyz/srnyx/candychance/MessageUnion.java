...
public class MessageUnion {
    ...
    @NotNull
    public RestAction<Message> sendMessage(@NotNull MessageEmbed embed) {
        if (isChannel()) return channel.sendMessageEmbeds(embed).map(sentMessage -> {
            messageId = sentMessage.getIdLong();
            return sentMessage;
        });
        if (isEvent()) return event.replyEmbeds(embed).complete().retrieveOriginal();
    ...
    public RestAction<?> editMessage(@NotNull String message) {
        if (isChannel()) {
            ...
            return channel.retrieveMessageById(messageId).complete().editMessage(message);
        }
        if (isEvent()) return event.getHook().editOriginal(message);
        throw new IllegalStateException("MessageUnion is not a channel or event");
    }
    ...
    public RestAction<?> editEmbed(@NotNull MessageEmbed embed) {
        if (isChannel()) {
            if (messageId == null) throw new IllegalStateException("MessageUnion has not sent a message");
            return channel.retrieveMessageById(messageId).complete().editMessageEmbeds(embed);
        }
        if (isEvent()) return event.getHook().editOriginalEmbeds(embed);
        throw new IllegalStateException("MessageUnion is not a channel or event");
    }
}
